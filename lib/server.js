...
var server = http.createServer(function (request, response) {
  ...
  if(url.pathname.match(/^\/access$/)) {
    ...
    var oauth_callback_url = "http://"+host+"/oauth_callback?host="+targetHost+"&js="+encodeURIComponent(url.query ? url.query.js : '');
    ...
    if(!cont) {
      ...
      cont(oauth_token_verifier, function (error, requester) {
        if(error) {
          ...
          var target = "http://"+url.query.host+"/oauth_transfer?oauth_token="+url.query.oauth_token+"&js="+encodeURIComponent(url.query.js);
